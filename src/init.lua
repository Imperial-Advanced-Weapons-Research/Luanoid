...
local Luanoid = Class() do
    function Luanoid:init(luanoidParams)
        ...
        self.ToolEquipped = Event()
        self.ToolUnequipping = Event()
    ...
    function Luanoid:MountRig(rig)
        assert(typeof(rig) == "Instance", "Expected Instance as Argument #1")
    ...
    function Luanoid:EquipTool(tool, handAttachmentName)
        assert(typeof(tool) == "Instance" and tool:IsA("Tool"), "Expected model as Argument #1")

        if self.Rig then
            local handAttachment = self.Character:FindFirstChild(handAttachmentName, true)
            local toolHandler = require(tool.Parent)

            tool = tool:Clone()
            toolHandler = toolHandler(tool, self)

            self.Tools[tool] = toolHandler
            toolHandler.RootPart.CFrame = handAttachment.WorldCFrame * toolHandler.GripOffset

            local weldConstraint = Instance.new("WeldConstraint")
            weldConstraint.Part0 = handAttachment.Parent
            weldConstraint.Part1 = toolHandler.RootPart
            weldConstraint.Parent = toolHandler.RootPart

            tool.Parent = self.Character
        end

        self.Character:SetAttribute("Hand".. handAttachmentName, tool.Name)
        self.ToolEquipped:Fire(tool, handAttachmentName)
    end

    function Luanoid:UnequipTool(handAttachmentName)
        local tool = self.Character[self.Character:GetAttribute("Hand".. handAttachmentName)]
        local toolHandler = self.Tools[tool]

        if toolHandler.Unequip then
            toolHandler:Unequip()
        end
        tool:Destroy()

        self.ToolUnequipping:Fire(tool, handAttachmentName)
        self.Character:SetAttribute("Hand".. handAttachmentName, nil)
    end

    function Luanoid:UnequipAllTools()
        for handAttachmentName in pairs(self.Character:GetAttributes()) do
            if handAttachmentName:sub(1, 4) == "Hand" then
                self:UnequipTool(handAttachmentName)
            end
        end
    end
    ...
    function Luanoid:SetNetworkOwner(networkOwner)
        assert(networkOwner == nil or (typeof(networkOwner) == "Instance" and networkOwner:IsA("Player")), "Expected nil or Player as Argument #1")
