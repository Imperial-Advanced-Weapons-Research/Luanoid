...
local function toggleMotorSet(model, enable, motorSet)
	local motors = {}
...
function Rigging.toggleMotors(model, enable, rigType)
	...
	local motors
	if rigType == Enum.HumanoidRigType.R6 then
		motors = toggleMotorSet(model, enable, R6_MOTOR6DS)
	elseif rigType == Enum.HumanoidRigType.R15 then
		motors = toggleMotorSet(model, enable, R15_MOTOR6DS)
	else
		error("unknown rig type", 2)
	end
